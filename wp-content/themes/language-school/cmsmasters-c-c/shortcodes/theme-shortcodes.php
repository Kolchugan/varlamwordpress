	...
	function cmsmasters_learnpress($atts, $content = null) {
		...
		if ($query->have_posts()) : 
			...
			$out .= "<article id=\"lpr_course_post-" . $course_id . "\" class=\"lpr_course_post\">" . "\n" . 
				...
				if (get_post_meta( $course_id, '_lp_price', true ) > 0 ) {
					$out .= "<div class=\"cmsmasters_course_price\">" . learn_press_get_currency_symbol() . floatval( get_post_meta( $course_id, '_lp_price', true ) ) . "</div>";
				} else {
